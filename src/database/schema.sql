...
-- ========================
-- CUSTOMER BAN / DEVICE TRACKING (idempotent)
-- ========================
-- Marks a user as banned. When true, `requireAuth` in the API rejects the
-- session and the storefront should redirect the user to the sign-in page.
ALTER TABLE users ADD COLUMN IF NOT EXISTS is_banned boolean NOT NULL DEFAULT false;
ALTER TABLE users ADD COLUMN IF NOT EXISTS banned_at timestamp with time zone;
ALTER TABLE users ADD COLUMN IF NOT EXISTS banned_reason text;
ALTER TABLE users ADD COLUMN IF NOT EXISTS banned_by uuid REFERENCES auth.users(id) ON DELETE SET NULL;

-- Records every browser/device that has ever authenticated as a given user.
-- `device_id` is a UUID generated by the storefront client and stored in a
-- long-lived cookie so a single browser keeps a stable identity across visits.
...
-- Every banned device_id lives here. The middleware rejects any authenticated
-- request whose device cookie is in this table.
...
-- RLS: no client-side access. Only the service-role key (admin API routes)
